...
export default function TeamPhotoUpdateForm(props) {
  const {
    id: idProp,
    teamPhoto: teamPhotoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = teamPhotoRecord
      ? { ...initialValues, ...teamPhotoRecord }
      : initialValues;
    setBucket(cleanValues.bucket);
    setThumbnail(cleanValues.thumbnail);
    setErrors({});
  };
  const [teamPhotoRecord, setTeamPhotoRecord] =
    React.useState(teamPhotoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TeamPhoto, idProp)
        : teamPhotoModelProp;
      setTeamPhotoRecord(record);
    };
    queryData();
  }, [idProp, teamPhotoModelProp]);
  React.useEffect(resetStateValues, [teamPhotoRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TeamPhoto.copyOf(teamPhotoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TeamPhotoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Bucket"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || teamPhotoModelProp)}
